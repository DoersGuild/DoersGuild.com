<?php
    ...
    $cache_path = "ai-cache";
    // where to store the generated re-sized images. Specify from your document root!
    $jpg_quality = 75;
    // the quality of any generated JPGs on a scale of 0 to 100
    $sharpen = TRUE;
    // Shrinking images can blur details, perform a sharpen on re-scaled images?
    $watch_cache = TRUE;
    // check that the adapted image isn't stale (ensures updated source images are re-cached)
    ...
    /* Override $_COOKIE with $_REQUEST, if set */
    if (isset($_REQUEST['resolution'])) {
        $_COOKIE['resolution'] = $_REQUEST['resolution'];
    }
    ...
    $resolution = isset($_REQUEST['target_width']) && is_numeric($_REQUEST['target_width']) ? intval($_REQUEST['target_width']) : $resolution;
